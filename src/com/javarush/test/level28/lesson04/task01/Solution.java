...
public class Solution {
    ...
    public static class AmigoThreadFactory implements ThreadFactory{
        ...
        static final AtomicInteger atomicInteger2 = new AtomicInteger(1);

        ThreadGroup tg = new ThreadGroup(atomicInteger2.getAndIncrement()+"");

        @Override
        public Thread newThread(final Runnable r)
        {

            return new Thread(tg.getParent().getName() +"-pool-" + tg.getName() + "-thread-" + atomicInteger.getAndIncrement()){
                @Override
                public void run()
                {
                    r.run();
