...
public class Solution
{
    ...
    public static boolean isNormalLockOrder(final Solution solution, final Object o1, final Object o2) throws Exception
    {
        ...
        thread0.start();
        thread1.start();
        Thread.sleep(90);  //даем время для thread1 заблокироватся, но не больше 100 мс

        boolean b = Thread.State.BLOCKED.equals(thread1.getState());

        //    System.out.println(b);   //вывод результата
        thread0.interrupt();
        thread1.interrupt();
        return b;
    }
    ...
    //    Thread.sleep(500);  //если убрать перерыв, то результат непредсказуем, так как эти объекты используются ранее
        new Thread()
        {
            @Override
            public void run()
